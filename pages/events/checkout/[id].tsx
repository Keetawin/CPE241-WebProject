...
const CheckoutPage = ({ event, available_seat, userPayment }: Props) => {
  ...
  if (parsedTicketType && Array.isArray(parsedTicketType)) {
    totalQuantity = parsedTicketType.reduce(
  ...
  useEffect(() => {
    ...
    ticket.forEach((t, idx) => {
      const ticket_info = parsedTicketType.find(
        (type) => type.seat_type_id === t.seat_type_id
      );
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://ticketapi.fly.dev/create_booking",
        body
      );
      const bookingId = response.data.booking_id;
      router.push(`/events/payment/${bookingId}`);
    } catch (error) {
      console.error(error);
      // Handle error here
    }
  ...
  return (
    <div className="container mx-auto px-10">
        ...
        <div className="p-4 flex ml-10">
          <div className="">
            ...
            <h3 className="text-gray-500 text-sm font-medium mb-3">
              Date: {dayjs(event.event_startdate).format("DD/MM/YYYY")} -{" "}
              {dayjs(event.event_enddate).format("DD/MM/YYYY")}
            </h3>
      ...
      {ticket &&
        ticket.map((t, idx) => {
          return (
            ...
            >
              <div className="flex">
                <div className="bg-[#E90064] w-8 h-8 flex justify-center items-center rounded-lg text-white font-semibold text-lg mr-2">
                  {idx + 1}
                </div>
                <h2 className="text-xl font-semibold">
                  {
                    parsedTicketType.find(
                      (type) => type.seat_type_id === t.seat_type_id
                    ).name
                  }
                </h2>
      ...
      <Button
        onClick={() => {
          router.push("/users/payment/add");
        }}
        variant="contained"
        color="primary"
        style={{ backgroundColor: "#E90064", marginTop: "2rem" }}
      >
        Add New Payment
      </Button>
        ...
        <div className="mt-6">
          <div className="border-2 border-gray-200 p-4 bg-white shadow rounded">
            ...
            <div className="flex justify-end">
              <span className="font-semibold">Subtotal</span>
              <span className="font-semibold ml-2">{billing.subTotal}</span>
            </div>
            <div className="flex justify-end">
              <span className="font-semibold">Voucher</span>
              <span className="font-semibold ml-2">{billing.voucher}</span>
            </div>
            <div className="flex justify-end">
              <span className="font-semibold">Refundable Tickets</span>
              <span className="font-semibold ml-2">{billing.refundDown}</span>
            </div>
      ...
      <Button
        disabled={
          !ticket.every(
            (ticket) => ticket.ticket_date !== "" && ticket.seat_no !== ""
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  try {
    const response1 = await axios.get(
      `https://ticketapi.fly.dev/get_event?event_id=${id}`
    );
    const response2 = await axios.get(
      `https://ticketapi.fly.dev/get_available_seat?event_id=${id}`
    );
